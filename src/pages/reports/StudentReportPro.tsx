          ...
          {shownFits.length > 0 && (
            <div className="mt-5">
              ...
              <div className="mt-2 rounded-lg border border-brand/25 bg-brand/5 p-3">
                <div className="mb-1 flex items-center gap-1.5 text-[11.5px] font-bold text-brand">
                  <Target className="h-3.5 w-3.5" /> كيف تُحسب نسبة المطابقة (الترشيح)؟
                </div>
                <div className="rounded-md bg-white/70 px-3 py-1.5 text-center text-[12px] font-semibold text-slate-800">
                  نسبة المطابقة = المتوسط الموزون للمعايير + مكافأة الخبرة (حتى +٣)
                </div>
                <p className="mt-1.5 text-[11px] leading-6 text-slate-600">
                  <b>المتوسط الموزون</b> = مجموع (درجة الطالب في كل معيار × وزن المعيار في المهمة) ÷ ١٠٠، فالمعيار الأعلى وزنًا يؤثّر أكثر.
                  المعايير المعروضة داخل كل بطاقة تُبيّن درجة الطالب فيها ووزنها. وهي نفس النسبة الظاهرة في تقرير المهمة وتفاصيلها.
                </p>
              </div>
            </div>
          )}
          ...
          <details className="slis-glossary mt-6 rounded-lg border border-slate-200 bg-slate-50/70 p-4">
            ...
            <dl className="mt-2 grid gap-x-5 gap-y-2 sm:grid-cols-2">
              {[
                ...
                ["نسبة المطابقة (المواءمة)", "مصطلح واحد لنتيجة معادلة الترشيح: متوسط معايير التقييم الموزون بأوزان المهمة + مكافأة خبرة (حتى +٣). تُحسب بنفس الطريقة في كل التقارير، فالرقم واحد لنفس الطالب والمهمة أينما ظهر."],
