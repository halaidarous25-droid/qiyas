...
export function scoreAssessment(answers: Answers): AssessmentResult {
  ...
  AXES.forEach((a) => {
    const items = QUESTIONS.filter((q) => q.type === "scenario" && q.axis === a.key);
  ...
  // 2) السلوك (المواقف)
  const sit = QUESTIONS.filter((q) => q.type === "situation" && answers[q.id] !== undefined);
  ...
  const ind = (kind: "integrity" | "emotional") => {
    const items = QUESTIONS.filter((q) => q.type === "indicator" && q.indicator === kind && answers[q.id] !== undefined);
  ...
  // 4) درجة التناقض (البنود الموازية مقابل سيناريوهاتها)
  let contradiction = 0;
  QUESTIONS.filter((q) => q.type === "parallel").forEach((p) => {
    const aP = answers[p.id]; const scen = byId(p.parallelTo!);
    const aS = answers[scen.id];
    if (typeof aP !== "number" || typeof aS !== "number" || !p.freqLevels) return;
    const level = p.freqLevels[aP];
    const scenScore = optScore(scen, aS);
    const gap = Math.abs(level - scenScore);
    contradiction += gap <= 20 ? 0 : gap <= 45 ? 1 : 2;
  });

  // 5) الكمال الاجتماعي (فخاخ صحيح/خطأ) — "صحيح" = نقطة
  const socialDesirability = QUESTIONS.filter(
